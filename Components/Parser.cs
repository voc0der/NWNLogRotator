...
namespace NWNLogRotator.Components
{
    ...
    "</head>";
            ...
            NWNLog = Regex.Replace(NWNLog, @"\[CHAT WINDOW TEXT\]", "", RegexOptions.Compiled);
            NWNLog = Regex.Replace(NWNLog, @"\[{1}[A-z]{3}\s[A-z]{3}\s[0-9]{2}\s", "<span class='timestamp'>[", RegexOptions.Compiled);
            NWNLog = Regex.Replace(NWNLog, @"\:{1}[0-9]*]{1}", "]</span>", RegexOptions.Compiled);
            // actors
            NWNLog = Regex.Replace(NWNLog, @"\]<\/span>((...).*: )", "]</span><span class='actors'>$1</span>", RegexOptions.Compiled);
            // tells
            NWNLog = Regex.Replace(NWNLog, @":\s?<\/span>\s?(\[Tell])(.*.*)", "</span><span class='tells'> $1:$2</span><br />", RegexOptions.Compiled);
            // whispers 
            NWNLog = Regex.Replace(NWNLog, @":\s?<\/span>\s?(\[Whisper])(.*.*)", "</span><span class='whispers'> $1:$2</span><br />", RegexOptions.Compiled);
            // emotes 
            NWNLog = Regex.Replace(NWNLog, @"(\*.*\*)", "<span class='emotes'>$1</span>", RegexOptions.Compiled);
            // html formatting
